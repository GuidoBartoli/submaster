...
class SrtTests(unittest.TestCase):
    ...
    def test_normalize_srt_resegments_zero_duration_oversized_cues(self) -> None:
        """Verify that generated zero-length oversized cues do not break normalization."""
        raw = (
            "1\n"
        ...
        self.assertGreater(len(cues), 1)
        for cue in cues:
            self.assertEqual(cue.start_ms, 57_000)
            self.assertEqual(cue.end_ms, 57_000)
            self.assertLessEqual(len(cue.text), 2)
            for line in cue.text:
                self.assertLessEqual(len(line), 42)
